...
- **a.** verbal nouns ending with _-ní/tí_  --> inherit the value from the verb  (like _Milovala **nakupování.activity**_; _Na naše **dotázání (se).performance** neodpověděli._) 

- **b.** other nouns --> <mark>`process`</mark> (like _After the **game.process**, she went.performance home._; _Na náš **dotáz.process** neodpověděli._)    



**2. events that are repeated / happen in general:**

- **a.** events that **iterate over multiple instances** spread out over time <u>for a given participant</u> (i.e., they occur normally or usually) --> <mark>`habitual`</mark> (as in  _Pekař **peče.habitual** housky_)  

- <mark>**NEW b.**</mark> events that are **hallmark characteristic of a class of entities** (i.e., over <u>an entire class of participants</u> (happens in general) (new value)) --> <mark>`generic`</mark>   
  
  (we use it also, e.g., for agentive nouns as *volič* = (any) person serving as ARG0 of the verb ***volit.generic***)

- **c.** ??? `iterative' for repeating perfective verbs, as in *světlo několikrát krátce **zablikalo.iterative***???



**3. NOT processes:** 

events that do not imply changes (like statives and similar verbs) --> <mark>`state`</mark> (_He **likes.stat** fish and chips, she **prefers.state** cheesburgers._)



**4. PROCESSES:**  

- **a. processes, imperfective verbs** (morphologically)  --><mark> `activity` </mark>(by default),  
  or `habitual` (if the denoted event occurs usually/repeatably);  
  also `performance` may be considered (with stressed result)   

- **b. processes, perfective verb** (morphologically) for verbs denoting process that comes to an end --><mark> `performance` </mark>(by default);  
  also<mark> `endeavor` </mark>may be considered (if it is clear that the process stopped/was interrupted without reaching a result state)   

- **c. processes, double aspect verbs** --> <mark>`process` </mark>(as a back-up) if it is hard/ impossible to distinguish 
...
aspectual verbs (~ fázová slovesa) --> merged with the lexical verb, aspect value <mark>`inceptive`</mark>
